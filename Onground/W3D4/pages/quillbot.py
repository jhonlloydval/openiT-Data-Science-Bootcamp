...
.pill  { background: rgba(255,255,255,0.12); color: #d4e8f7; padding: 4px 13px;
         border-radius: 999px; font-size: 0.8rem; border: 1px solid rgba(255,255,255,0.2); }
.card  { background: #fff; border: 1px solid #e2eaf4; border-radius: 14px; padding: 22px; margin-bottom: 18px; }
.slabel { font-size: 0.75rem; font-weight: 700; color: #4a6785;
          text-transform: uppercase; letter-spacing: 0.05em; margin-bottom: 8px; }
.output-box { background: #f7fafd; border: 1.5px solid #b8d4f0; border-radius: 12px;
              padding: 20px; line-height: 1.8; white-space: pre-wrap;
              color: #0f1f33; font-size: 0.97rem; }
...
.err  { background: #fff5f5; border: 1px solid #f5b8b8; border-radius: 10px;
        padding: 12px 16px; color: #7d2a2a; font-size: 0.88rem; }
...
<div class="header">
  <h1>✦ QuillBot</h1>
  <p>AI writing assistant powered by llama3.2 · Paraphrase · Grammar check · Review</p>
  <div class="pills">
    <span class="pill">Paraphrase</span>
    <span class="pill">Grammar check</span>
    <span class="pill">AI review</span>
    <span class="pill">Tone adjustment</span>
  </div>
</div>
""", unsafe_allow_html=True)
...
with col_side:
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown('<div class="slabel">Tool</div>', unsafe_allow_html=True)
    mode = st.radio(
        "mode",
        ["Paraphrase", "Grammar check", "AI review"],
        index=0,
        label_visibility="collapsed",
    )
    st.markdown('<div class="slabel" style="margin-top:14px">Tone</div>', unsafe_allow_html=True)
    ...
    st.caption("llama3.2 · localhost:11434")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if not text or not text.strip():
        ...
        with st.spinner("QuillBot is thinking…"):
            ...
            try:
                ...
                st.markdown(
                    f'<div class="output-box">{reply.replace(chr(10), "<br>")}</div>',
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    '<div class="err">❌ Cannot reach Ollama. Make sure it is running at localhost:11434.</div>',
                    unsafe_allow_html=True,
                )
            except requests.exceptions.Timeout:
                st.markdown(
                    '<div class="err">⏱️ Request timed out. Try again or use a shorter text.</div>',
                    unsafe_allow_html=True,
                )
